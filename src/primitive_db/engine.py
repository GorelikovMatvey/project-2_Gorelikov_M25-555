...
from .parser import parse_insert, parse_set_where, parse_where_condition
from .utils import (
    load_metadata,
    load_table_data,
    save_metadata,
        ...
        try:
            if command == "create_table":
                ...
                if len(args) > 3 and args_lower[3] == "where":
                    try:
                        # Парсим условие WHERE с поддержкой разных операторов
                        where_clause = parse_where_condition(args[4:])
                ...
                try:
                    where_clause = parse_where_condition(
                        args[where_index + 1:])
                except ValueError as e:
                    print(f"Ошибка в условии WHERE: {e}")
                    continue
