...
def compare_values(value1, value2, operator: str) -> bool:
...
@handle_db_errors
@log_time
def select(table_data: list[dict], where: tuple | None = None) -> list[dict]:
    """Выбирает записи с кэшированием.

    where: tuple(column, operator, value) или None
    """
    ...
    def compute():
        ...
        for row in table_data:
            if column in row and compare_values(row[column], value, operator):
                results.append(row)
        return results
...
@handle_db_errors
def update(
        table_data: list[dict], set_clause: dict, where: tuple) -> list[dict]:
...
def delete(table_data: list[dict], where: tuple) -> list[dict]:
    ...
    table_data = [row for row in table_data
                  if not (column in row and compare_values(row[column], value,
                                                           operator))]
